"""Sync sheet data to DB - with truncation for column limits"""
...
# Column size limits from the model
MAX_LEN = {
    'developer_name': 255, 'status_detail': None, 'technology_detail': 100,
    'rtb_date': 20, 'cod_date': 20, 'commune': 200, 'department': 100,
    'department_code': 10, 'region': 100, 'permit_type': 20, 'permit_status': 50,
    'grid_operator': 50, 'offtake_type': 100,
}
...
for row in data_rows:
    ...
    if v:
        m = re.match(r'(.+)\((\d+)\)', v)
        if m:
            updates['department'] = trunc(m.group(1).strip(), 'department')
            updates['department_code'] = trunc(m.group(2), 'department_code')
    
    # Col 11: Région
    v = row[11].strip() if len(row) > 11 and row[11] else ''
    if v:
        updates['region'] = trunc(v, 'region')
    
    # Col 12: MWc
    v = row[12].strip() if len(row) > 12 and row[12] else ''
    if v and v != 'ex: 5':
        try: updates['capacity_mw'] = float(v)
        except: pass
    
    # Col 17: P50
    v = row[17].strip() if len(row) > 17 and row[17] else ''
    if v and v != 'ex: 15 600':
        try: updates['p50_mwh'] = float(v.replace(' ', '').replace(',', '.'))
        except: pass
    ...
    if v and v not in ('(PC / DP / ICPE)', ''):
        updates['permit_type'] = trunc(v.strip(' ()'), 'permit_type')
    
    # Col 32: Statut permis (improved logic: check exact phrases)
    v = row[32].strip() if len(row) > 32 and row[32] else ''
    if v and v not in ('(À déposer / Déposé / Accepté / Recours / Purge)', ''):
        ps = v.lower().replace('\n', ' ').strip()
        if 'purgé' in ps or 'purged' in ps or 'caa' in ps:
            updates['permit_status'] = 'purgé'
        elif 'recours' in ps:
            updates['permit_status'] = 'recours'
        elif 'déposé' in ps or 'deposé' in ps:
            updates['permit_status'] = 'déposé'
        elif 'déposer' in ps or 'deposer' in ps:
            updates['permit_status'] = 'à déposer'
    
    # Col 36: GRD
    v = row[36].strip() if len(row) > 36 and row[36] else ''
    if v and v not in ('(Enedis / RTE / Autre)', ''):
        updates['grid_operator'] = trunc(v.strip(' ()'), 'grid_operator')
    ...
    if v and v not in ('(PPA / OA / AO CRE / Autre)', ''):
        updates['offtake_type'] = trunc(v.replace('\n', ' ').strip(), 'offtake_type')
    
    # Col 49: Developer fallback
    v = row[49].strip() if len(row) > 49 and row[49] else ''
    if v and 'developer_name' not in updates:
        updates['developer_name'] = trunc(v, 'developer_name')
    
    if updates:
        set_clause = ', '.join(f"{k} = :{k}" for k in updates)
        updates['id'] = proj_id
        conn.execute(text(f"UPDATE projects SET {set_clause} WHERE id = :id"), updates)
        print(f"  ✅ Code {code}: {len(updates)-1} fields")
        updated += 1

conn.commit()
conn.close()
print(f"\n=== Synced: {updated} projects ===")
