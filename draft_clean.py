...
# Delete old broken draft
try:
    service.users().drafts().delete(userId='me', id='r3125813147860643224').execute()
    print("✅ Old broken draft deleted")
except:
    pass
...
for fpath, fname in files:
    if os.path.exists(fpath):
        ...
        # Use proper encoding for the filename
        part.add_header('Content-Disposition', 'attachment', filename=fname)
        part.add_header('Content-Type', 'application/octet-stream', name=fname)
        msg.attach(part)
        print(f"✅ {fname}")
    else:
...
print(f"Vérification : {len(parts)} pièces jointes")
for p in parts:
    fn = p.get('filename', '(vide)')
